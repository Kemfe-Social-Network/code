...
class U extends Controller
{
  public function index($value = '')
  {
    ...
    if (isset($_COOKIE['auth']) && !empty($_COOKIE['auth'])) {
      ...
      $lUser = my_decrypt($_COOKIE['auth'], $key);
    }

    $id = get_user_id_by_username($value);
    $sql = "SELECT COUNT(espals_community_post.id) AS posts, user.* , COUNT(distinct  t.id) AS following, COUNT( distinct f.id) AS followers, COUNT(DISTINCT w.id) as isFollowing ";
    ...
    $sql .= " LEFT JOIN espals_follow_system w ON f.user = '{$lUser}' AND w.following = '{$id}' ";
    $sql .= " WHERE user.user_id = '{$value}'  ";
    $data_user_data1 = $db->fetchQuery($db->query($sql));


    $this->view('u/index', $data_user_data1);
  }
  ...
  public function fetch_post($type)
  {
    ...
    global $db;
    $id = get_user_id_by_username($catName[0]);
    if ($catName[2] == "post") {
      if (isset($_COOKIE['auth']) && !empty($_COOKIE['auth'])) {
        $key = KEY;
      ...
      $sql .= $left;
      $sql .= " WHERE espals_community_vote.post_id IN ($tags) ";
      $match = false;
